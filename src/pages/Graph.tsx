...
import { type Cell, Edge, type Node, Graph as X6Graph } from "@antv/x6";
...
const FlowEditor: React.FC = () => {
        ...
        img.onload = () => {
          ...
          if (cropTop || cropRight || cropBottom || cropLeft) {
            ...
            ctx.rect(
              cropLeft,
              cropTop,
              canvasWidth - cropLeft - cropRight,
              canvasHeight - cropTop - cropBottom,
            );
            ctx.clip();
          }
  ...
  const handleValidateFlow = () => {
    ...
    const startNodes = nodes.filter(
      (node) => node.attr("body/fill") === "#52c41a",
    );
    ...
    const endNodes = nodes.filter(
      (node) => node.attr("body/fill") === "#ff4d4f",
    );
  ...
  const nodePaletteItem = (type: NodeType, active: boolean) => {
    ...
    const clipPath = (() => {
      ...
      const pts = body.refPoints
        .split(" ")
        .map((p) => p.split(",").map((n) => Number(n)));
      const poly = pts
        .map(([x, y]) => `${(x / cfg.width) * 100}% ${(y / cfg.height) * 100}%`)
        .join(",");
      return `polygon(${poly})`;
    })();
    const style: React.CSSProperties = {
      ...
      fontSize: Math.max(
        12,
        Math.round((cfg.attrs.label.fontSize ?? 14) * scale),
      ),
      userSelect: "none",
          ...
          <div style={{ width: 120 }}>
            <svg width={120} height={24}>
              ...
              <line
                ...
                markerEnd={
                  line.targetMarker ? `url(#arrow-${type})` : undefined
                }
              />
            </svg>
          </div>
  ...
  const getCellValue = (cell: Cell, path: string) => {
    ...
    if (path === "width")
      return cell.isNode() ? (cell as Node).size().width : undefined;
    if (path === "height")
      return cell.isNode() ? (cell as Node).size().height : undefined;
    ...
    if (path === "router/name" && cell.isEdge())
      return (cell as Edge).getRouter()?.name;
    if (path === "connector/name" && cell.isEdge())
      return (cell as Edge).getConnector()?.name;
    return undefined;
  };
  ...
  const setCellValue = (cell: Cell, path: string, value: unknown) => {
    if (path === "label") {
      if (cell.isEdge()) {
        (cell as Edge).setLabels([
          { attrs: { label: { text: String(value ?? "") } } },
        ]);
      } else {
  ...
  const handleContextMenuClick: MenuProps["onClick"] = ({ key }) => {
    ...
    switch (key) {
      ...
      case "edit": {
        ...
        const fields: Record<string, unknown> = { type };
        const props = contextCell.isEdge()
          ? edgeEditableProps
          : nodeEditableProps;
        props.forEach((p) => {
      ...
      >
        ...
        >
          <div style={{ display: "grid", gridTemplateColumns: "1fr", gap: 8 }}>
            {nodeTypes.map((type) =>
              nodePaletteItem(type, placingType === type),
            )}
          </div>
        </Card>
        ...
        >
          <div style={{ display: "grid", gridTemplateColumns: "1fr", gap: 8 }}>
            {edgeTypes.map((type) =>
              edgePaletteItem(type, placingEdgeType === type),
            )}
          </div>
        </Card>
          ...
          >
            ...
            <div style={{ display: "flex", gap: 8 }}>
              <InputNumber
                ...
                max={4000}
                value={canvasWidth}
                onChange={(v) => setCanvasWidth(Number(v) || 0)}
              />
              <InputNumber
                addonBefore="高"
                min={100}
                max={4000}
                value={canvasHeight}
                onChange={(v) => setCanvasHeight(Number(v) || 0)}
              />
            </div>
          </div>
        </Card>
        ...
        >
          ...
          >
            ...
            <div style={{ display: "flex", alignContent: "center", gap: 8 }}>
              <ColorPicker
                value={bgColor}
                onChange={(_, hex) => setBgColor(hex)}
              />
              <Input
                placeholder="#hex 或 rgb() 或 hsl()"
                value={bgColor}
                onChange={(e) => setBgColor(e.target.value)}
              />
            </div>
            <Input
              placeholder="linear-gradient(to right, #fff, #eee)"
              value={bgGradient}
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              ...
              <InputNumber
                ...
                step={0.05}
                value={bgOpacity}
                onChange={(v) => setBgOpacity(Number(v) || 0)}
              />
              <InputNumber
                addonBefore="缩放"
                min={0.1}
                ...
                step={0.1}
                value={bgScale}
                onChange={(v) => setBgScale(Number(v) || 1)}
              />
            </div>
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              <InputNumber
                ...
                max={2000}
                value={bgPosX}
                onChange={(v) => setBgPosX(Number(v) || 0)}
              />
              <InputNumber
                addonBefore="Y"
                min={-2000}
                max={2000}
                value={bgPosY}
                onChange={(v) => setBgPosY(Number(v) || 0)}
              />
            </div>
            <div style={{ display: "grid", gap: 8, flexDirection: "column" }}>
              <InputNumber
                ...
                max={canvasHeight}
                value={cropTop}
                onChange={(v) => setCropTop(Number(v) || 0)}
              />
              <InputNumber
                addonBefore="右"
                min={0}
                max={canvasWidth}
                value={cropRight}
                onChange={(v) => setCropRight(Number(v) || 0)}
              />
              <InputNumber
                addonBefore="下"
                min={0}
                max={canvasHeight}
                value={cropBottom}
                onChange={(v) => setCropBottom(Number(v) || 0)}
              />
              <InputNumber
                addonBefore="左"
                min={0}
                max={canvasWidth}
                value={cropLeft}
                onChange={(v) => setCropLeft(Number(v) || 0)}
              />
            </div>
          </Space>
        </Card>
        ...
        >
          <div style={{ display: "flex", gap: 8, flexDirection: "column" }}>
            ...
            />
            <Button
              onClick={() => handleExportImage("png", 0.92)}
              icon={<DownloadOutlined />}
            >
              下载PNG
            </Button>
            <Button
              onClick={() => handleExportImage("jpeg", 0.92)}
              icon={<DownloadOutlined />}
            >
              下载JPEG
            </Button>
          ...
          <div style={{ marginBottom: 8, fontSize: 12, color: "#666" }}>
            {placingEdgeType && (
              <span style={{ color: "#1890ff" }}>
                点击两个节点以创建连接线（当前：{placingEdgeType}）
              </span>
            )}
            {placingType && (
              <span style={{ color: "#1890ff" }}>
                点击空白处以放置节点（当前：{getNodeLabel(placingType)}）
              </span>
            )}
          </div>
          <div
