export type NodeType = 'start' | 'task' | 'decision' | 'end' | 'data' | 'connector' | 'process' | 'database' | 'manual' | 'auto';
export type EdgeType = 'default' | 'arrow' | 'dashed' | 'orth' | 'manhattan' | 'smooth' | 'bezier';
...
export const nodeTypes: NodeType[] = ['start', 'task', 'decision', 'end', 'data', 'connector', 'process', 'database', 'manual', 'auto'];
export const edgeTypes: EdgeType[] = ['default', 'arrow', 'dashed', 'orth', 'manhattan', 'smooth', 'bezier'];
...
export const edgeEditableProps = [
  ...
  { path: 'labels/0/attrs/label/fontSize', label: '标签字体大小', type: 'number' },
  { path: 'router/name', label: '路由算法', type: 'string' },
  { path: 'connector/name', label: '连接器', type: 'string' },
] as const;

export const getNodeDefaults = (type: NodeType): NodeConfig => nodeDefaults[type] || nodeDefaults.task;
export const getEdgeDefaults = (type: EdgeType): EdgeConfig => edgeDefaults[type] || edgeDefaults.default;