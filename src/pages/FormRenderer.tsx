...
const componentRender = (cfg: FieldConfig) => {
  ...
  if (cfg.type === "number")
    return <InputNumber style={{ width: "100%" }} {...cfg.props} />;
  if (cfg.type === "select")
    return (
      <Select style={{ width: "100%" }} options={cfg.options} {...cfg.props} />
    );
  if (cfg.type === "radio")
    return <Radio.Group options={cfg.options} {...cfg.props} />;
  ...
  if (cfg.type === "date")
    return <DatePicker style={{ width: "100%" }} {...cfg.props} />;
...
const FormRenderer: React.FC<{ config?: FieldConfig[] }> = ({
  config = [],
}) => {
  const [formCfg, setFormCfg] = useState<FormConfig>({
    layout: "grid",
    gutter: 16,
    cols: 24,
    items: [],
  ...
  const onItemDragStart = (index: number, e: React.DragEvent) => {
    dragIndexRef.current = index;
  };
  const onItemDragOver = (index: number, e: React.DragEvent) => {
    e.preventDefault();
  };
  ...
  const genReactCode = () => {
    ...
    lines.push(
      "import { Form, Input, InputNumber, Select, Radio, Switch, DatePicker, Button, Row, Col } from 'antd'",
    );
    ...
    if (formCfg.layout === "grid") {
      ...
      formCfg.items.forEach((it) => {
        ...
        lines.push(
          `<Form.Item label="${it.label}" name="${it.field}" rules={${JSON.stringify(toAntRules(it.rules))}}>\n`,
        );
  ...
  const genVueCode = () => {
    ...
    tpl.push("<form>");
    formCfg.items.forEach((it) => {
      tpl.push(`<div><label>${it.label}</label><input /></div>`);
    });
    tpl.push("</form>");
    tpl.push("</div>");
    tpl.push("</template>");
  ...
  return (
    ...
    >
      <Card
        ...
        headStyle={{ position: "sticky", top: 0, background: "#fff" }}
        bodyStyle={{ overflowY: "auto", scrollBehavior: "smooth", padding: 8 }}
      >
        <Space direction="vertical" style={{ width: "100%" }}>
          {palette.map((p) => (
            <div
              key={p.id}
              draggable
              onDragStart={(e) => onPaletteDragStart(p, e)}
              style={{
      ...
      >
        <Space style={{ marginBottom: 8 }}>
          <Select
            value={formCfg.layout}
            onChange={(v) => setFormCfg((prev) => ({ ...prev, layout: v }))}
            options={[
              { label: "栅格", value: "grid" },
              { label: "弹性", value: "flex" },
            ]}
          ...
          <InputNumber
            ...
            min={0}
            max={32}
            value={formCfg.gutter}
            onChange={(v) =>
              setFormCfg((prev) => ({ ...prev, gutter: Number(v) || 0 }))
            }
          />
        </Space>
        <div
        ...
        >
          {formCfg.items.map((it, idx) => (
            <div
              key={it.id}
              draggable
              onDragStart={(e) => onItemDragStart(idx, e)}
              onDragOver={(e) => onItemDragOver(idx, e)}
              onDrop={(e) => onItemDrop(idx, e)}
              style={{
                border:
                  selected === it.id
                    ? "1px solid #1890ff"
            ...
            >
              <Space style={{ marginBottom: 8 }}>
                ...
                <InputNumber
                  ...
                  max={24}
                  value={it.span || 12}
                  onChange={(v) => updateItem(it.id, { span: Number(v) || 12 })}
                />
        ...
        {(() => {
          ...
          return (
            <Space direction="vertical" style={{ width: "100%" }}>
              <Select
                value={it.type}
                onChange={(v) => updateItem(it.id, { type: v })}
                options={[
              ...
              <div>
                ...
                <Space direction="vertical" style={{ width: "100%" }}>
                  <Space>
                    <Switch
                      checked={Boolean(
                        (it.rules || []).find((r) => r.required),
                      )}
                      onChange={(v) => {
                        const rules = [...(it.rules || [])].filter(
                          (r) => !r.required,
                        );
                  ...
                  <Input
                    placeholder="正则"
                    value={
                      (it.rules || []).find((r) => r.pattern)?.pattern || ""
                    }
                    onChange={(e) => {
                      const rules = [...(it.rules || [])].filter(
                        (r) => !r.pattern,
                      ...
                      if (e.target.value)
                        rules.push({ pattern: e.target.value });
                      updateItem(it.id, { rules });
                    }}
                  />
                  <Input
                    placeholder="提示"
                    value={
                      (it.rules || []).find((r) => r.message)?.message || ""
                    }
                    onChange={(e) => {
                      const rules = [...(it.rules || [])].filter(
                        (r) => !r.message,
                      ...
                      if (e.target.value)
                        rules.push({ message: e.target.value });
                      updateItem(it.id, { rules });
                    }}
                  />
                </Space>
        ...
        <Form layout="vertical">
          {formCfg.layout === "grid" ? (
            <div
              style={{
                display: "grid",
                gridTemplateColumns: `repeat(${Math.max(1, Math.floor((formCfg.cols || 24) / 12))}, 1fr)`,
                gap: formCfg.gutter,
              }}
            >
              {formCfg.items.map((it) => (
                <div
                  key={it.id}
                  style={{
                    gridColumn: `span ${Math.max(1, Math.round((it.span || 12) / 12))}`,
                  }}
                >
                  <Form.Item
                    label={it.label}
                    name={it.field}
                    rules={toAntRules(it.rules)}
                  >
                    {componentRender(it)}
                  </Form.Item>
                </div>
      ...
      >
        <Space>
          <Button
            onClick={() => downloadText("GeneratedForm.tsx", genReactCode())}
          >
            下载React代码
          </Button>
          <Button
            onClick={() => downloadText("GeneratedForm.vue", genVueCode())}
          >
            下载Vue代码
          </Button>
        </Space>
